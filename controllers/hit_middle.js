...
module.exports = {
    ...
    checkout_middle(req, res) {
       ...
       console.log(checkno);
        ...
        let promise2=((checkno)=>{
            let ans_set;
            return new Promise((resolve,reject)=>{
            Guess_middle.find({a15waterno:checkno}, function(err, guesslist) {
        ...
        setTimeout(()=>{
            res.redirect("/api/hit_middle/?statusreport="+statusreport)
            //res.end()
        },3000)                      
    },//EOF checkout
    //創建 
    ...
    list(req, res) {
        ...
        Hit_middle.find({},null,{sort: {a10waterno:-1,a05guess_ids:-1,a25ans_set:1}},function(err, hitlist) {
            if (err)
            res.send(err);
            //res.json(hit);
            res.render("hit_middle/listpage",{
